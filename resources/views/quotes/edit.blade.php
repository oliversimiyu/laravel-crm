...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-white">Edit Quote</h1>
        ...
        <form action="{{ route('quotes.update', $quote) }}" method="POST">
            ...
            <div class="bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="quote_number" class="block text-sm font-medium text-gray-300 mb-1">Quote Number</label>
                        <input type="text" name="quote_number" id="quote_number" value="{{ old('quote_number', $quote->quote_number) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="issue_date" id="issue_date" value="{{ old('issue_date', $quote->issue_date->format('Y-m-d')) }}" required
                               class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @error('issue_date')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $quote->expiry_date->format('Y-m-d')) }}" required
                               class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @error('expiry_date')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="tax_rate" id="tax_rate" value="{{ old('tax_rate', $quote->tax_rate) }}" min="0" max="100" step="0.01"
                               class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @error('tax_rate')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_amount" id="discount_amount" value="{{ old('discount_amount', $quote->discount_amount) }}" min="0" step="0.01"
                               class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $quote->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-lg font-semibold text-white">Quote Items</h2>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        let itemCount = {{ count($quote->items) }};
